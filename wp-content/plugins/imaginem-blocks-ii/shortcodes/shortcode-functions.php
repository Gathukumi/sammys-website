...
if ( !function_exists( 'imaginem_codepack_activate_lightbox' ) ) {
	function imaginem_codepack_activate_lightbox ($lightbox_type="default",$ID="",$predefined=false,$mediatype="image",$title="",$class="",$set=false,$data_name="default", $external_thumbnail_id =false, $imageDataID = false, $proofing = false ) {
        ...
        $desc = '';
        $disable_lightbox_onlydesc = imaginem_codepack_get_option_data( 'disable_lightbox_onlydesc' );
        if ( false === $disable_lightbox_onlydesc ) {
            $featuredimg_desc = '';
        }
        $disable_lightbox_onlypurchase = imaginem_codepack_get_option_data( 'disable_lightbox_onlypurchase' );
        if ( false === $disable_lightbox_onlypurchase ) {
            $purchase_tag = '';
        }
		if ( $featuredimg_desc<>"" || $purchase_tag<>"" ) {
			$desc =  '<div class="lightbox-text entry-content">'.$featuredimg_desc.$purchase_tag.'</div>';
        }
        $disable_lightbox_title = imaginem_codepack_get_option_data( 'disable_lightbox_onlytitle' );

        $the_title_tag = '<h4 class="lightbox-text lightbox-text-title">'.$featuredimg_title.'</h4>';
        if ( false === $disable_lightbox_title ) {
            $the_title_tag = '';
        }

		$html_subtext = '<div class="lightbox-text-wrap '.$purchase_link_present.'">'.$the_title_tag.$desc.'</div>';

		$output = false;
		$like_data = '';
		$like_tag = '';
		if ( $proofing ) {
			$like_data = '<span id="lightbox-like-id-'.$featured_image_id.'" class="lightbox-like-holder animated" data-lightboxlike="'.$featured_image_id.'"><span class="mtheme-like like-vote-icon"><i class="proofing-lightbox-like proofing-lightbox-clickready ion-ios-heart-outline"></i></span></span>';
			$like_tag = 'data-title="'.esc_attr($like_data).'"';
		}
		if ( isSet($link) && $link<>"" ) {
			$output='<a data-elementor-open-lightbox="no" data-exthumbimage="'.esc_url( $thumbnail_link ).'" '.$gallery.'class="lightbox-active '.$class.'" data-sub-html="'.esc_attr( $like_data . $html_subtext ).'" href="'.esc_url( $link ).'" '.$data_name.'="'.$link.'">';
		}
		return $output;
	}
}
...
if ( !function_exists( 'imaginem_codepack_get_option_data' ) ) {
    function imaginem_codepack_get_option_data( $field_id, $default_value = '' ) {
        ...
        if ( function_exists( 'blacksilver_get_option_data' ) ) {
            $default = blacksilver_get_option_data( $field_id, $default_value );
            return $default;
        } else {
            return false;
        }
