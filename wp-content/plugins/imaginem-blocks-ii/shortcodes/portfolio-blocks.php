    ...
    function mtheme_MetroGrids($atts, $content = null)
    {
        ...
        if ($filter_image_ids) {
            ...
            foreach ($filter_image_ids as $attachment_id) {
                ...
                $thumbnails .= '<img class="preload-image displayed-image" src="' . $thumbnail_imageURI . '" alt="' . imaginem_codepack_get_alt_text($attachment_id) . '">';
                $thumbnails .= '</a>';
                $thumbnails .= '</div>';
                $thumbnails .= '</div>';
            }
            ...
            foreach ($filter_image_ids as $attachment_id) {
                ...
                if (isSet($thumbnail_tags) && is_array($thumbnail_tags)) {
                    foreach ($thumbnail_tags as $key => $value) {
                        $thumbnail_tag_slug                   = $value->term_id;
                ...
                if ( isSet($thumbnail_imagearray[0]) && $thumbnail_imagearray[0]<>"" ) {
                    if ($lazyload=="true") {
                        ...
                        $thumbnails .= '<img class="preload-image lazyload displayed-image" src="'. esc_url( $fallback_image ) .'" data-src="' . $thumbnail_imagearray[0] . '" alt="' . imaginem_codepack_get_alt_text($attachment_id) . '">';   
                    } else {
...
if (!function_exists('mPortfolioGrids')) {
    function mPortfolioGrids($atts, $content = null)
    {
        ...
        if ( 'asc' !== $sort && 'desc' !== $sort && 'custom' !== $sort ) {
            $sort = 'default';
        }
        if ( 'asc' === $sort ) {
            $sort_order = 'ASC';
        ...
        $newlink_tag = '';
        if ( 'newtab' === $open_newlink ) {
            $newlink_tag = ' target="_blank"';
        }
            ...
            while (have_posts()):
                ...
                if (is_array($portfolio_cats)) {
                    foreach ($portfolio_cats as $taxonomy) {
                        $portfoliogrid .= 'filter-' . $taxonomy->term_id . ' ';
                        if ($pagination == 'true') {
                ...
                if ($type != "ajax") {
                    ...
                    if ($portfolio_link_type <> "") {
                        ...
                        if ($portfolio_link_type == "Lightbox_DirectURL") {
                            $portfoliogrid .= '<a ' . $newlink_tag . ' class="column-gridblock-icon" href="' . get_permalink() . '">';
                        ...
                        switch ($portfolio_link_type) {
                            case 'DirectURL':
                                $the_only_link = true;
                                $portfoliogrid .= '<a ' . $newlink_tag . ' class="gridblock-sole-link" href="' . get_permalink() . '">';
                                if (post_password_required()) {
                            ...
                            case 'Customlink':
                                $the_only_link = true;
                                $portfoliogrid .= '<a ' . $newlink_tag . ' class="gridblock-sole-link" href="' . esc_url( $customlink_URL ) . '">';
                ...
                if ($style == "classic" ) {
                    if ($title == 'true' || $desc == 'true') {
                        ...
                        if ($title == 'true') {
                            if ($type != "ajax") {
                                $portfoliogrid .= '<h4><a ' . $newlink_tag . ' href="' . $hreflink . '">' . get_the_title() . '</a></h4>';
                            } else {
                                $portfoliogrid .= '<h4>';
        ...
        if ($type == "filter" || $type == "ajax") {
            ...
            foreach ($categories as $category) {
                ...
                if (!isSet($all_works) || in_array($term_slug, $all_works)) {
                    ...
                    $filter_portfoliogrid .= '<a data-filter=".filter-' . $category->term_id . '" data-title="' . $category->name . '" href="#">';
                ...
                foreach ($filter_image_ids as $attachment_id) {
                    ...
                    if (isSet($thumbnail_imageURI)) {
                        ...
                        if ( 'link' === $lightbox ) {
                            ...
                            $portfoliogrid .= '<a ' . $linktarget . ' class="column-gridblock-lightbox lightbox-image" href="' . esc_url( $link_url ) . '">';
                        }
                        if ( 'purchase' === $lightbox ) {
                            $purchase_url   = get_post_meta($attachment_id, 'mtheme_attachment_purchase_url', true);
                            $portfoliogrid .= '<a class="column-gridblock-lightbox lightbox-image" href="' . esc_url( $purchase_url ) . '">';
                        }
                        $portfoliogrid .= '<div class="gridblock-background-hover">';
                        ...
                        if ( 'purchase' === $lightbox ) {
                            ...
                            $icon_class = '<i class="' . imaginem_codepack_get_portfolio_icon('purchase') . '"></i>';
                        ...
                        if ( $lightbox == 'true' || 'link' === $lightbox || 'purchase' === $lightbox ) {
                            $portfoliogrid .= '</a>';
                        }
                        $count_lightboxes++;
        ...
        if (have_posts()):
            while (have_posts()):
                ...
                if ( $grid_post_type == "blog" ) {
                    ...
                    if (isset($custom['pagemeta_client_desc'][0])) {
                        $description = $custom['pagemeta_client_desc'][0];
                    }                    
                } else {
                    if (isset($custom['pagemeta_thumbnail_desc'][0])) {
                        $description = $custom['pagemeta_thumbnail_desc'][0];
                    ...
                    if ($title == 'true' || $desc == 'true') {
                        ...
                        if ($grid_post_type == "events") {
                            ...
                            if ( $when_display == 'yes' ) {
                                ...
                                $portfoliogrid .= '<div class="worktype-categories">';
                                $portfoliogrid .= $start_date;
                                if ( $start_date !== $end_date ) {
                                    $portfoliogrid .= '<span class="date-seperator"> - </span>';
                                    $portfoliogrid .= $end_date;
                                }
                                $portfoliogrid .= '</div>';
                                $portfoliogrid .= '</div>';
                            }
