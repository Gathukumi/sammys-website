...
class Imaginemcore_Demo_Importer {
	...
	public function import_files() {
		return array(
			...
			array(
				'import_file_name'           => 'Titan',
				'categories'                 => array(),
				'local_import_file'          => plugin_dir_path( __FILE__ ) . 'demo-data/titan/blacksilver-titan.xml',
				'local_import_widget_file'   => plugin_dir_path( __FILE__ ) . 'demo-data/titan/blacksilver-tian-widgets.json',
				'import_customizer_file_url' => plugins_url( 'demo-import/demo-data/titan/blacksilver-titan-customizer.dat', dirname(__FILE__) ),
				'import_preview_image_url'   => plugins_url( 'demo-import/demo-data/titan/titan-screenshot.jpg', dirname(__FILE__) ),
				'import_notice'              => __( 'This process will import <strong>Titan Demo</strong>.<br/><br/>If the demo import is not successful then it is likely due to server settings. Let us know through our <a style="box-shadow: none !important;" target="_blank" href="https://imaginemthemes.co/">Support Forum</a> for us to help.', 'themecore' ),
				'preview_url'                => '',
			),
			array(
				'import_file_name'           => 'Titan Dark',
				'categories'                 => array(),
				'local_import_file'          => plugin_dir_path( __FILE__ ) . 'demo-data/titan-dark/blacksilver-titan-dark.xml',
				'local_import_widget_file'   => plugin_dir_path( __FILE__ ) . 'demo-data/titan-dark/blacksilver-tian-dark-widgets.json',
				'import_customizer_file_url' => plugins_url( 'demo-import/demo-data/titan-dark/blacksilver-titan-dark-customizer.dat', dirname(__FILE__) ),
				'import_preview_image_url'   => plugins_url( 'demo-import/demo-data/titan-dark/titan-dark-screenshot.jpg', dirname(__FILE__) ),
				'import_notice'              => __( 'This process will import <strong>Titan Dark Demo</strong>.<br/><br/>If the demo import is not successful then it is likely due to server settings. Let us know through our <a style="box-shadow: none !important;" target="_blank" href="https://imaginemthemes.co/">Support Forum</a> for us to help.', 'themecore' ),
				'preview_url'                => '',
			),
			array(
				'import_file_name'           => 'Spektra',
				'categories'                 => array(),
				'local_import_file'          => plugin_dir_path( __FILE__ ) . 'demo-data/spektra-bright/spektra-bright.xml',
				'local_import_widget_file'   => plugin_dir_path( __FILE__ ) . 'demo-data/spektra-bright/spektra-bright-widgets.json',
				'import_customizer_file_url' => plugins_url( 'demo-import/demo-data/spektra-bright/spektra-bright-customizer.dat', dirname(__FILE__) ),
				'import_preview_image_url'   => plugins_url( 'demo-import/demo-data/spektra-bright/screenshot.jpg', dirname(__FILE__) ),
				'import_notice'              => __( 'This process will import <strong>Spektra Bright Demo</strong>.<br/><br/>If the demo import is not successful then it is likely due to server settings. Let us know through our <a style="box-shadow: none !important;" target="_blank" href="https://imaginemthemes.co/">Support Forum</a> for us to help.', 'themecore' ),
				'preview_url'                => '',
			),
			array(
				'import_file_name'           => 'Spektra Dark',
				'categories'                 => array(),
				'local_import_file'          => plugin_dir_path( __FILE__ ) . 'demo-data/spektra-dark/spektra-dark.xml',
				'local_import_widget_file'   => plugin_dir_path( __FILE__ ) . 'demo-data/spektra-dark/spektra-dark.json',
				'import_customizer_file_url' => plugins_url( 'demo-import/demo-data/spektra-dark/spektra-customizer.dat', dirname(__FILE__) ),
				'import_preview_image_url'   => plugins_url( 'demo-import/demo-data/spektra-dark/screenshot.jpg', dirname(__FILE__) ),
				'import_notice'              => __( 'This process will import <strong>Spektra Dark Demo</strong>.<br/><br/>If the demo import is not successful then it is likely due to server settings. Let us know through our <a style="box-shadow: none !important;" target="_blank" href="https://imaginemthemes.co/">Support Forum</a> for us to help.', 'themecore' ),
				'preview_url'                => '',
			),
		);
	}
	...
	public function after_import_setup( $selected_import ) {
		...
		if ( 'Onepage' === $selected_import['import_file_name'] ) {
			...
			$this->assign_centauri_pages();
			$this->assign_centauri_menus();
		} elseif ( 'Titan' === $selected_import['import_file_name'] ) {
			$this->assign_titan_pages();
			$this->assign_titan_menus();
		} elseif ( 'Titan Dark' === $selected_import['import_file_name'] ) {
			$this->assign_titan_pages();
			$this->assign_titan_dark_menus();
		} elseif ( 'Spektra' === $selected_import['import_file_name'] ) {
			$this->assign_spektra_pages();
			$this->assign_spektra_menus();
		} elseif ( 'Spektra Dark' === $selected_import['import_file_name'] ) {
			$this->assign_spektra_pages();
			$this->assign_spektra_menus();
		} else {
			$this->assign_pages();
	...
	public function assign_spektra_menus() {
		$locations = get_theme_mod( 'nav_menu_locations' );
		$menus     = wp_get_nav_menus();

		if($menus) {
			foreach($menus as $menu) {
				if( $menu->name == 'Spektra Menu' ) {
					$locations['main_menu'] = $menu->term_id;
					$locations['mobile_menu'] = $menu->term_id;
				}
			}
	...
	public function assign_titan_menus() {
		$locations = get_theme_mod( 'nav_menu_locations' );
		$menus     = wp_get_nav_menus();

		if($menus) {
			foreach($menus as $menu) {
				if( $menu->name == 'Titan Menu' ) {
					$locations['main_menu'] = $menu->term_id;
					$locations['mobile_menu'] = $menu->term_id;
				}
			}
	...
	public function assign_titan_dark_menus() {
		$locations = get_theme_mod( 'nav_menu_locations' );
		$menus     = wp_get_nav_menus();

		if($menus) {
			foreach($menus as $menu) {
				if( $menu->name == 'TitanMenu Dark' ) {
					$locations['main_menu'] = $menu->term_id;
					$locations['mobile_menu'] = $menu->term_id;
				}
			}
	...
	public function assign_titan_pages() {
	...
	public function assign_spektra_pages() {
