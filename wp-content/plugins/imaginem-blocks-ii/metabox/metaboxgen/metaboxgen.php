		...
		if ( isSet($field['type']) ) {
			...
			case 'selected_proofing_images':
					...
					if ( $found_selection ) {
						...
						foreach ( $filter_image_ids as $attachment_id) {
							...
							if ($proofing_status=="true") {
								...
								echo '<li class="images"><img src="'.esc_url($thumbnail_imageURI).'" alt="'.esc_attr__('selected','imaginem-blocks-ii').'" /></li>';
								$found_selection = true;
							}
						}
						foreach ( $filter_image_ids as $attachment_id) {
				...
				if (!$found_selection) {
					...
					_e('No selection found.','imaginem-blocks-ii');
					echo '</div>';
				}
			...
			case 'image_gallery':
				...
				$thumbs_output = '';
				$button_text = ($meta) ? esc_html__('Edit Gallery', 'imaginem-blocks-ii') : $field['std'];
				$renew_meta = '';
				if( $meta ) {
					$field['std'] = esc_html__('Edit Gallery', 'imaginem-blocks-ii');
					$thumbs = explode(',', $meta);
					$thumbs_output = '';
					$imageidcount = 0;
				...
				$metagallery_nonce = wp_create_nonce( 'themecore-nonce-metagallery' );
			    echo 
			    	'<td>
			    		<input type="button" class="button" name="' . esc_attr( $field['id'] ) . '" id="mtheme_images_upload" value="' . esc_attr($button_text) .'" />
			    		
			    		<input type="hidden" name="mtheme_meta[_mtheme_image_ids]" id="_mtheme_image_ids" data-pageid="'.get_the_ID().'" data-nonce="'.$metagallery_nonce.'" value="' . esc_attr($renew_meta ? $renew_meta : 'false') . '" />
				...
				case 'display_image_attachments' :
					...
					if ($images) {
						foreach ( $images as $id => $image ) {
							...
							echo '<img src="'. esc_url( $imageURI ).'" alt="'.esc_attr__('image','imaginem-blocks-ii').'" />';
						}
					} else {
						echo esc_html__('No images found.','imaginem-blocks-ii');
					}
					break;
			...
			// Color picker
				...
				case 'repeat_text':
					...
					if (isSet($meta) && is_array($meta) ) {
						foreach ($meta['size'] as $value) {
							if (isSet($value) && $value<>"") {
								...
								echo '<input placeholder="'.esc_attr__('Size','imaginem-blocks-ii').'" type="text" name="'. esc_attr($field['id']).'[size][]" value="'. esc_attr($size) .'" id="box_size'.$repeat_count.'" />';
								echo '<input placeholder="'.esc_attr__('Price','imaginem-blocks-ii').'" type="text" name="'. esc_attr($field['id']).'[price][]" value="'. esc_attr($price) .'" id="box_price'.$repeat_count.'" />';
								if ($repeat_count>0) {
									echo '<span class="remove-box">'.esc_html__('Remove','imaginem-blocks-ii').'</span>';
								}
								echo '</div>';
							}
							$repeat_count++;
					...
					if (!$found_data) {
						echo '<div class="text-box" id="text-box">';
						echo '<input placeholder="'.esc_attr__('Size','imaginem-blocks-ii').'" type="text" name="'. esc_attr($field['id']).'[size][]" value="" id="box_size0" />';
						echo '<input placeholder="'.esc_attr__('Price','imaginem-blocks-ii').'" type="text" name="'. esc_attr($field['id']).'[price][]" value="" id="box_price0" />';
						echo '</div>';
					}
					echo '</div>';
					echo '<span class="add-box">'.esc_html__('Add more','imaginem-blocks-ii').'</span>';
					echo '<span class="add-box-notice">'.esc_html__('Max Reached!','imaginem-blocks-ii').'</span>';
					echo '</div>';
					break;
				case 'timepicker':
					$text_value = $meta ? $meta : $field['std'];
				...
				case 'fontselector':
					...
					$googlefont_text = __('abcdefghijklmnopqrstuvwxyz ABCDEFGHIJKLMNOPQRSTUVWXYZ 0123456789','imaginem-blocks-ii');
				...
				case 'select':
					...
					foreach ($field['options'] as $key => $option) {
						if ($key=='0') {
							$key = __('All the items','imaginem-blocks-ii');
						}
						...
						if ($field['target']=="client_names") {
							if ( get_post_type($meta) == 'clients' ) {
								if( themecore_has_password($meta) ){
									...
									echo esc_html__('Client selected has password protection.','imaginem-blocks-ii');
									echo '<br/><strong>';
									echo esc_html__('Gallery password protected.','imaginem-blocks-ii');
									echo '</strong></div>';
								} else {
									echo '<div class="metabox-notice metabox-notice-no-pass">';
									echo esc_html__('Client selected does not have password protection.','imaginem-blocks-ii');
									echo '<br/>';
									echo esc_html__('The gallery will be available for everyone.','imaginem-blocks-ii');
									echo '<br/><br/>';
									echo esc_html__('Add a password to the Client page to protect the gallery.','imaginem-blocks-ii');
									echo '</div>';
								}
							}
						}
