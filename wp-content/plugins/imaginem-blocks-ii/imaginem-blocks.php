 ...
 * Text Domain: imaginem-blocks-ii
 */
...
function themecore_Imaginem_Blocks_load() {
	...
	load_plugin_textdomain( 'imaginem-blocks-ii' );
...
function themecore_Imaginem_Blocks_fail_load_out_of_date() {
	...
	$message = '<p>' . __( 'Elementor Imaginem Blocks is not working because you are using an old version of Elementor.', 'imaginem-blocks-ii' ) . '</p>';
	$message .= '<p>' . sprintf( '<a href="%s" class="button-primary">%s</a>', $upgrade_link, __( 'Update Elementor Now', 'imaginem-blocks-ii' ) ) . '</p>';
...
function themecore_add_elementor_section_background_controls( \Elementor\Element_Section $section ) {
    ...
    $section->add_control(
        ...
        [
            'label' => __( 'Parallax <br/>For non-dynamic background.<br/>Disables Size/Position', 'imaginem-blocks-ii' ),
            'type' => \Elementor\Controls_Manager::SWITCHER,
            'label_off' => __( 'Off', 'imaginem-blocks-ii' ),
            'label_on' => __( 'On', 'imaginem-blocks-ii' ),
            'default' => 'no',
        ]
    );
}
        ...
        if(isset($page) && $page->get_id() > "") {
            ...
            if ($mtheme_post_type == 'page') {
                ...
                $page->add_control(
                    'pagemeta_pagestyle',
                    [
                        'label' => __( 'Sidebar', 'imaginem-blocks-ii' ),
                        'type' => \Elementor\Controls_Manager::SELECT,
                        'default' => 'edge-to-edge',
                        'options' => [
                            'leftsidebar'   => __( 'Left', 'imaginem-blocks-ii' ),
                            'nosidebar'     => __( 'No Sidebar', 'imaginem-blocks-ii' ),
                            'rightsidebar'  => __( 'Right', 'imaginem-blocks-ii' ),
                            'edge-to-edge'  => __( 'Edge to Edge', 'imaginem-blocks-ii' ),
                        ],
                    ]
                );
            }
...
function themecore_add_rellax_controls( \Elementor\Element_Base $element) {
	...
	$element->start_controls_section(
		'section_rellax',
		[
			'label' => __( 'Rellax', 'imaginem-blocks-ii' ),
	...
	$element->add_control(
		...
		[
			...
			'type' => \Elementor\Controls_Manager::SWITCHER,
			'description' => __( 'Uncheck this switch if you want to disable Rellax.', 'imaginem-blocks-ii' ),
            'label_off' => __( 'Off', 'imaginem-blocks-ii' ),
            'label_on' => __( 'On', 'imaginem-blocks-ii' ),
            'default' => 'no',
		]
    ...
    $element->add_control(
        'rellax_speed',
        [
            'label' => __( 'Speed', 'imaginem-blocks-ii' ),
